...
defineComponents(SimpleGreeting); // Component will be registered as <simple-greeting>
```

By default, the kebab-case version of the class name will be used as tag name.
You can also specify an explicit tag name for your component:

```tsx
// highlight-next-line
class SimpleGreeting extends Component("x-greeting", {
  name: prop<string>("John"),
}) {
  render() {
    return <h1>Hello, {this.props.name}!</h1>;
...
defineComponents(SimpleGreeting); // Component will be registered as <x-greeting>
```

:::warning

Note that tag names of web components must include a dash `-`.
